...
### Open question - does Battle need a HUD row at all?
...
numeric total, e.g. "848"/"956") and unit/opponent name labels. The Figma reference shows a HUD row
here; the current game doesn't.

This is a product question, not a styling one: is the omission intentional (Battle is combat-focused,
Round/Gold/Triumph aren't relevant mid-fight, only squad HP matters) or a genuine gap to fill? Don't
add a HUD row here without checking - it may have been deliberately left off to reduce clutter during
the fastest-paced screen in the app. Flag this in the PR rather than deciding it either way.
...
3. Do not touch the timer ring, threat meter, synergy banner, or losing hint - all four are already
   correctly implemented.
4. Raise the HUD-row question in the PR description; don't add one without product sign-off.
