...
- **Always state which repo the task is for:** every hand-off must open by naming the target repo explicitly - `syn-grid-client` (this repo) or `syn-grid` (the Go server, at `../syn-grid`) - since Cursor may be pointed at either checkout and must not guess from context.
